...
def select_support_or_resistance(data, timeframe_days, timeframe_key, is_support, distance_threshold):
    ...
    # 第二階段：降級尋找（使用Tier 1，無論距離）
    if tier1_candidates:
        if is_support:
            # 支撐：選擇最高的Tier 1候選
            best = max(tier1_candidates, key=lambda x: x[1])
        else:
            # 壓力：選擇最低的Tier 1候選
            best = min(tier1_candidates, key=lambda x: x[1])
...
def analyze_stock(symbol):
    ...
    for timeframe_days, timeframe_key in [(5, 'period_5'), (20, 'period_20'), (60, 'period_60')]:
        ...
        if support:
            print(f"  支撐: {support['value']:.2f} (來源: {support['source']}, {support['type']}, 距離: {support['distance']:.2f}%)")
        else:
            print(f"  支撐: 無")

        if resistance:
            print(f"  壓力: {resistance['value']:.2f} (來源: {resistance['source']}, {resistance['type']}, 距離: {resistance['distance']:.2f}%)")
        else:
            print(f"  壓力: 無")
